...
void CController::ProcessC2Thread()
{
	while (keep_running)
	{
		...
		switch (packet->GetCodecIn())
		{
			...
			case ECodecType::dmr:
			case ECodecType::audio:
				// codec_in was AMBE or Audio, so we need to calculate the the M17 data
...
void CController::Dump(const std::shared_ptr<CTranscoderPacket> p, const std::string &title) const
{
	...
	line << title << " #" << p->GetSequence() << " Mod='" << p->GetModule() << "' SID=" << std::showbase << std::hex << ntohs(p->GetStreamId()) << std::noshowbase << " ET:" << std::setprecision(3) << p->GetTimeMS();

	ECodecType in = p->GetCodecIn();
	if (p->AudioIsSet())
		line << " Audio";
	if (ECodecType::audio == in)
		line << '*';
	if (p->DStarIsSet())
		line << " DStar";
