...
CTranscoderPacket::CTranscoderPacket(const STCPacket &tcp) : audio_set(false), dstar_set(false), dmr_set(false), m17_set(false), not_sent(true)
{
	...
	case ECodecType::audio:
		SetAudioSamples(tcp.audio, false);
		break;
...
void CTranscoderPacket::SetAudioSamples(const int16_t *sample, bool swap)
{
	for (unsigned int i=0; i<160; i++)
		tcpacket.audio[i] = swap ? ntohs(sample[i]) : sample[i];
	audio_set = true;
}

const int16_t *CTranscoderPacket::GetAudioSamples() const
{
	return tcpacket.audio;
}
...
bool CTranscoderPacket::AudioIsSet() const
{
	return audio_set;
}
...
{
	return (audio_set && dstar_set && dmr_set && m17_set);
}
